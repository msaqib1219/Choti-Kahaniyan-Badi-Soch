...
const StoryCard: React.FC<StoryCardProps> = ({ story, language, onComplete, onExit }) => {
  ...
  const displayTitle = isUrdu && story.titleUrdu ? story.titleUrdu : story.title;
  const displayMoral = isUrdu && story.moralUrdu ? story.moralUrdu : story.moral;
  const displayText = isUrdu && scene.textUrdu ? scene.textUrdu : scene.text;
  const displayInstruction = isUrdu && scene.action.instructionUrdu ? scene.action.instructionUrdu : scene.action.instruction;
  ...
  if (isFinished) {
    return (
      ...
      >
        ...
        <h2 className={`font-display text-4xl text-yellow-600 mb-4 ${isUrdu ? 'font-urdu' : ''}`}>
          {isUrdu ? 'شباش! بہت اچھے!' : 'Shabbash! Well Done!'}
        </h2>
        <p className="text-xl text-center mb-8 font-medium">
          {isUrdu ? 'مبارک ہو! آپ نے کہانی ختم کر لی:' : 'Mubarak! You finished the story:'} <br/>
          <span className={`font-bold text-2xl text-blue-600 ${isUrdu ? 'font-urdu' : ''}`}>"{displayTitle}"</span>
        </p>
        <div className="bg-blue-50 p-6 rounded-2xl mb-8 border-2 border-blue-200 italic">
          <p className={`text-lg text-blue-800 ${isUrdu ? 'font-urdu' : ''}`}>"{displayMoral}"</p>
        </div>
        <div className="flex gap-4">
          ...
          >
            <RotateCcw size={20} /> {isUrdu ? 'واپس جائیں' : 'Back Home'}
          </button>
        </div>
      </motion.div>
    );
  ...
  return (
    <div className={`max-w-4xl mx-auto w-full flex flex-col items-center gap-6 ${isUrdu ? 'rtl' : 'ltr'}`} dir={isUrdu ? 'rtl' : 'ltr'}>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="aspect-video w-full bg-slate-100 relative group overflow-hidden">
             ...
             {!interacted && renderTargets.map((pos, i) => (
                <motion.button
                  ...
                  className="absolute bg-white text-slate-800 p-6 rounded-full shadow-2xl animate-bounce hover:bg-slate-50 transition-all z-20 text-4xl flex items-center justify-center min-w-[80px] min-h-[80px] font-emoji"
                  style={{ 
                    left: `${pos.x}%`, 
                    top: `${pos.y}%`,
                  }}
          ...
          <div className="p-8 md:p-12">
            <div className={`story-text text-slate-700 mb-8 ${isUrdu ? 'font-urdu text-3xl' : ''}`}>
            ...
            <div className={`p-6 rounded-2xl transition-all duration-300 border-2 ${interacted ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-200 border-dashed'}`}>
               <div className="flex flex-col gap-2">
                 <p className={`text-xl font-bold flex items-center gap-3 ${interacted ? 'text-green-700' : 'text-slate-700'} ${isUrdu ? 'font-urdu' : ''}`}>
                    {interacted ? <Star size={24} fill="currentColor" /> : (isUrdu ? '👈' : '👉')} 
                    {displayInstruction}
                 </p>
      ...
      <div className="flex justify-between w-full px-4 mb-12">
        <button 
          onClick={onExit}
          className={`text-slate-500 hover:text-slate-800 font-bold transition-colors ${isUrdu ? 'font-urdu' : ''}`}
        >
          {isUrdu ? 'منسوخ کریں' : 'Cancel Story'}
        </button>
        ...
        {interacted && (
          <motion.button
            ...
            animate={{ opacity: 1, scale: 1 }}
            onClick={handleNext}
            className={`px-12 py-4 bg-yellow-400 text-yellow-900 font-display text-2xl rounded-full hover:bg-yellow-500 transition-all shadow-xl flex items-center gap-3 active:scale-95 ${isUrdu ? 'font-urdu' : ''}`}
          >
            {currentScene === story.scenes.length - 1 
              ? (isUrdu ? 'کہانی ختم!' : 'Finish Story!') 
              : (isUrdu ? 'اگلا صفحہ' : 'Next Page')} 
            <ArrowRight size={24} className={isUrdu ? 'rotate-180' : ''} />
          </motion.button>
        )}
      </div>
    </div>
